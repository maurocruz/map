...
export default function useInitialViewport() 
{
  ...
  useEffect(() => {
    const substringToExclude = Router.asPath.substring(0,2);
    if (substringToExclude === '/?') {
      const queryString = Router.asPath.slice(2);
      queryString.split('&').map((nameValueString) => {
        ...
        // LONGITUDE
        if (name == 'longitude' || name == 'lng') initialViewport.longitude = parseFloat(value);
        // LATITUDE
        if (name == 'latitude' || name == 'lat') initialViewport.latitude = parseFloat(value);
        // ZOOM
        if (name == 'zoom' || name == 'z') initialViewport.zoom = parseFloat(value);
        // PITCH
        if (name == 'pitch' || name == 'p') initialViewport.pitch = parseFloat(value);
        // BEARING
        if (name == 'bearing' || name == 'b') initialViewport.bearing = parseFloat(value);
      });
